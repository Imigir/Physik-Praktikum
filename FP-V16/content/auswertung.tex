...
\end{equation}
mit der Aktivität $A=\SI{15,13(22)}{\becquerel}$, die auf die Goldfolie trifft, der Teilchendichte $n$, der Dicke $d$ und dem Raumwinkel $\Omega=\frac{\pi r^2}{R^2}$, der vom Detektor abgedeckt wird. Es ergeben sich die Werte aus Tabelle \ref{tab:dataDeg}. Der Fehler auf $N$ ergibt sich wegen der Poisson-Verteilung zu $\sigma_N= \sqrt{N}$.
...
Mit Formel \ref{eq:Rutherford} ergibt sich daraus
\begin{align*}
\left(\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega}\right)_.{\SI{2}{\micro\meter}}&=\SI{8,61(23)e-23}{\meter^2}\\
\left(\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega}\right)_.{\SI{4}{\micro\meter}}&=\SI{2,90(9)e-23}{\meter^2}
\end{align*}
...
Die Daten für die Dicken $d$ der Folien und der Intensitäten werden in diesem Versuchsteil zur Verfügung gestellt. Es werden die mit den Werten aus Tabelle \ref{tab:values} nach Formel \eqref{eq:Rutherford} berechneten differentiellen Wirkungsquerschnitte mit den theoretischen Werten bei $\theta=\SI{10}{\degree}$ verglichen. Diese berechnen sich nach Formel \eqref{eq:RSF}.\\
...
\caption{Der experimentelle und der theoretische differentielle Wirkungsquerschnitt $\frac{.d\sigma}{.d\Omega}$ aufgetragen gegen die Kernladungszahl $Z$.}
\label{fig:ZAbh}
\end{figure}

\begin{table}
	\centering
	\caption{Die Intensität $I_\theta$ für einen bestimmten Streuwinkel $\theta=\SI{10}{\degree}$, der experimentelle differentielle Wirkungsquerschnitt, sowie der theoretische differentielle Wirkungsquerschnitt für denselben Winkel in Abhängigkeit von der Kernladungszahl $Z$.}

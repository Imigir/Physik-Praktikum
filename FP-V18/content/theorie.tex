...
Trifft ein solches $\gamma$-Quant auf Materie, dominieren drei Arten von Wechselwirkungen dabei für unterschiedliche Energien. Die Wahrscheinlichkeit für jede Art von Wechselwirkung lässt sich durch einen Wirkungsquerschnitt $\sigma$ beschreiben, der anschaulich darstellt, dass die eingestrahlten Projektilteilchen ($\gamma$-Quant)auf ein ausgedehntes Ziel, ein Targetteilchen (Absorberatom), treffen.
In Abbildung \ref{fig:Wirkungsquerschnitt} ist die Abhängigkeit der Extinktionskoeffizienten $\mu=n\sigma$, mit der Anzahl der Elektronen pro Volumeneinheit $n$, der verschiedenen Wechselwirkungsarten von der Energie der $\gamma$-Quanten zu sehen.\\
Die Wahrscheinlichkeit $\diff W$, dass ein Teilchen von dem Absorber eingefangen wird, lässt sich mit der Absorberschichtdicke $dx$ durch
\begin{equation}
...
\]
Es gilt $4 < \alpha < 5$.  Der Exponent $\delta$ ist abhängig von der Photonenenergie. Für natürlich vorkommende $\gamma$-Strahlung gilt $\delta\approx 3,5$, für Energien $E_.{\gamma}\geq \SI{5}{\mega\electronvolt}$ sinkt er auf $\delta\approx 1$.
...
Der Compton-Effekt beschreibt die elastische Streuung eines $\gamma$-Quants an einem als in Ruhe angenommenen Hüllenelektron. Das $\gamma$-Quant gibt dabei nur einen Teil der Energie ab und fliegt danach mit veränderter Frequenz abgelenkt weiter. Eine schematische Darstellung davon ist in Abbildung \ref{fig:Compton} zu sehen. Geschieht dieser Effekt an einem Elektron auf einem äußeren Energieniveau, so kann dieses einen großen Teil der Photonenenergie aufnehmen und aus der Bindung herausgeschlagen werden. Bei stärker gebundenen Elektronen verbleibt das Elektron im gebundenen Zustand.
...
\end{equation}
Dabei bezeichnet $\sigma_.{Th}=\frac{8}{3}\pi\left(\frac{e_.0}{4\pi\varepsilon_.0c^2m_.e}\right)^2$, mit der Elementarladung $e_.0$, der Dielektrizitätskonstante $\varepsilon_.0$, der Lichtgeschwindigkeit im Vakuum $c$ und der Ruhemasse des Elektrons $m_.e$, Den Thomson-Wirkungsquerschnitt.
Der differentielle Wirkungsquerschnitt $\frac{.d\sigma}{.dE}$ lässt sich beschreiben durch:
\begin{equation}
\frac{\diff \sigma}{\diff E} =  \frac{\pi r_\text{e}^2}{m_0 c^2 \epsilon^2}\left(2+\left(\frac{E_\text{e'}}{E_\gamma - E_\text{e'}}\right)^2 \left[ \frac{1}{\epsilon^2} +\frac{E_\gamma - E_\text{e'}}{E_\gamma} - \frac{2(E_\gamma - E_\text{e'})}{\epsilon E_\gamma} \right]\right) \label{eq:sigmadiff}
\end{equation}

\begin{figure}
	\centering
	\includegraphics[width=\linewidth-70pt,height=\textheight-70pt,keepaspectratio]{content/images/Compton.jpg}
...
\begin{figure}
	\centering
	\includegraphics[width=\linewidth-70pt,height=\textheight-70pt,keepaspectratio]{content/images/Spektrum.jpg}
	\caption{Spektrum eines monochromatischen $\gamma$-Quants \cite{V18}.}
	\label{fig:Spektrum}
\end{figure}
